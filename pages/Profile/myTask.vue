	...
	<view class="container">
		...
		<view class="app-box">
			<block v-if="taskList.length > 0">
				<task-publish v-for="(item, index) in taskList" :key="item.task_id" :detail="item"></task-publish>

				<u-loadmore v-if="taskList.length > 3" :status="status" margin-top="50" color="#333" margin-bottom="30" :load-text="loadText" />
			</block>
			<block v-else>
				<z-empty text="暂无任务数据~"></z-empty>
			</block>
...
<script>
	import TaskPublish from '@/component/TaskPublish.vue'
	...
	export default {
		...
		methods: {
			...
			async getMyTask() {
				try {
					let res = await myTask(this.params)
